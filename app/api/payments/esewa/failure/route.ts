...
export async function GET(request: Request) {
  try {
    ...
    const { transaction_uuid, status = 'FAILED', message = 'Payment failed' } = responseBody;
    ...
    if (payment) {
      ...
      console.warn('No payment record found for transaction:', transaction_uuid);
    }
    
    // Redirect to the failed payment page with error information
    const failureUrl = new URL(`${url.origin}/dashboard/user/orders/failed`);
