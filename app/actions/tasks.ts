  ...
  try {
    ...
    const tasks = staff.appointments.map(appointment => {
      ...
      return {
        ...
        completedAt: appointment.status === "COMPLETED" ? new Date(appointment.updatedAt) : undefined,
        feedback: ""
      };
    });
...
export async function updateTaskStatus(taskId: string, status: string, completionNotes?: string) {
  try {
    ...
    console.log(`Updating task ${taskId} status to ${status} (DB status: ${mapToDbStatus(status)})`)
    ...
    const updatedAppointment = await prisma.appointment.update({
      ...
      data: {
        ...
        notes: completionNotes ? `${completionNotes}\n${new Date().toISOString()}` : undefined
      }
    })
